...
/**
 * Collects and displays paths to matched artifacts in depMgt tree, if exists.
 */
@CommandLine.Command(
        name = "dm-tree-find",
        description = "Collects and displays paths to matched artifacts in dm tree, if exists")
@Mojo(name = "gav-dm-tree-find", requiresProject = false, threadSafe = true)
public class GavDmTreeFindMojo extends GavMojoSupport {
    /**
    ...
    protected Result<List<List<Artifact>>> doExecute() throws Exception {
        ...
        return toolboxCommando.dmTreeFind(
